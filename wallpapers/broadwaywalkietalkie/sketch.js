...
function setup() {
    ...
    currentLF = lifeField;
    lastLF = lifeField2;
    currentLF[Math.floor(random(a))][Math.floor(random(b))] = 1;
}

//toggles cell at x, y
function flickCell(x, y) {
   if (currentLF[x][y] == 0) {
       currentLF[x][y] = Math.floor(random(6));
   }
   else {
       currentLF[x][y] = 0;
   }
    ...
    for(var i = 0; i < a; i++) {
        for(var j = 0; j < b; j++) {
            ...
            //fill cell that's filled
                ...
                else if (currentLF[i][j] == 4) {
                    fill(200, 20, 200);
                }
                else if (currentLF[i][j] == 5) {
                    fill(20, 200, 200);
                }
